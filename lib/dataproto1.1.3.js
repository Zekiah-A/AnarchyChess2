...
class DataReader extends DataView{
	constructor(arr){
...
class DataWriter extends Array{
	constructor(){
		super()
...
function registerTypes(dict){
	for(const k in dict){
		types[k] = dict[k]
		typesReverse.set(dict[k], k)
	}
}
const typeToJson = type => JSON.stringify(type, (_, v) => typeof v == 'object' || typeof v == 'number' || typeof v == 'string' ? v : typeof v == 'function' ? typesReverse.get(v) : undefined)
const jsonToType = json => JSON.parse(json, (_, v) => typeof v == 'object' || typeof v == 'number' ? v : typeof v == 'string' ? types[v] : undefined)